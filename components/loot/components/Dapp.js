...
class Dapp extends React.Component {
  ...
  render() {
    ...
    return (
      <>
        <div className={classnames(styles.app, 'w-96 h-96')}>
                ...
                <>
                  ...
                  <div className="card" style={{ backgroundColor: 'white', marginTop: '15px' }}>
                    <img
                      ...
                      src={this.state.displayedUser.character}
                    />
        ...
        <footer style={{ textAlign: 'center' }}>
          <div>
            Created by <a href="https://twitter.com/stephancill">@stephancill</a>
          </div>
          <div>
            Inspired by <a href="https://lootproject.com">lootproject.com</a>
          </div>
          <div>
            Source on <a href="https://github.com/stephancill/synthetic-loot-viewer">GitHub</a>
          </div>
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const { pathname: prevPathname, query: preQuery } = prevProps;
    
    if (query.addressOrENS) {
      if (!prevState.ready && this.state.ready) {
        this.updateUser(query.addressOrENS);
        return;
      }
      if (query.addressOrENS === preQuery.addressOrENS) {
        return;
      }
      this.updateUser(query.addressOrENS);
    }
  ...
  async updateUser(addressOrENS) {
    if (
      this.state.displayedUser &&
      (this.state.displayedUser.address === addressOrENS ||
        this.state.displayedUser.ens === addressOrENS)
    ) {
      return;
    }
