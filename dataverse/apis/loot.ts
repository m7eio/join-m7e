...
const apiKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweEEzOTU5MUNGOUExYUVDMzllNUVGNmExQzI2MDc5NDA5RUEyMDk3ODgiLCJpc3MiOiJuZnQtc3RvcmFnZSIsImlhdCI6MTYyNzU3MDY0MTAxMSwibmFtZSI6ImRhdGF2ZXJzZSJ9.iYqoQZc_N-pThgPH_c3saB54C1_7U3JY2K1d62ZcDWg';
...
export async function fetchLoot(address: string) {
  ...
  if (window.ethereum) {
    ...
    provider = new ethers.providers.JsonRpcProvider("https://mainnet.infura.io/v3/f29f1c340a60430ebff33f1ed9dad190");
  }
...
function getLayers(LOOT: any) {
  ...
  const LAYERS: any = {
    "fg": `${imgDir}/fg.png`, "bg": `${imgDir}/bg.png`,   // Foreground & Background are at the root
...
function updateLayers(loot_name: string, loot:string, LAYERS: any) {
  ...
  if (parsedName["suffix"]) {
    LAYERS[`${loot_name}Suffix`] = `${imgDir}/${mapping[loot_name]['suffix'][parsedName['name']][parsedName['suffix']]}`;
  }
